...
TEST_F(ExecutorTest, DISABLED_SimpleSeqScanTest) {
  ...
  TableMetadata *table_info =
      GetExecutorContext()->GetCatalog()->GetTable("test_1");
  ...
  auto *const500 =
      MakeConstantValueExpression(ValueFactory::GetIntegerValue(500));
  auto *predicate =
      MakeComparisonExpression(colA, const500, ComparisonType::LessThan);
  auto *out_schema = MakeOutputSchema({{"colA", colA}, {"colB", colB}});
  SeqScanPlanNode plan{out_schema, predicate, table_info->oid_};

  // Execute
  std::vector<Tuple> result_set;
  GetExecutionEngine()->Execute(&plan, &result_set, GetTxn(),
                                GetExecutorContext());

  // Verify
  std::cout << "ColA, ColB" << std::endl;
  for (const auto &tuple : result_set) {
    ASSERT_TRUE(tuple.GetValue(out_schema, out_schema->GetColIdx("colA"))
                    .GetAs<int32_t>() < 500);
    ASSERT_TRUE(tuple.GetValue(out_schema, out_schema->GetColIdx("colB"))
                    .GetAs<int32_t>() < 10);
    std::cout << tuple.GetValue(out_schema, out_schema->GetColIdx("colA"))
                     .GetAs<int32_t>()
              << ", "
              << tuple.GetValue(out_schema, out_schema->GetColIdx("colB"))
                     .GetAs<int32_t>()
              << std::endl;
  }
...
TEST_F(ExecutorTest, DISABLED_SimpleRawInsertTest) {
  ...
  std::vector<Tuple> result_set;
  GetExecutionEngine()->Execute(&scan_plan, &result_set, GetTxn(),
                                GetExecutorContext());
...
TEST_F(ExecutorTest, DISABLED_SimpleSelectInsertTest) {
  ...
  const Schema *out_schema1;
  {
    ...
    scan_plan1 = std::make_unique<SeqScanPlanNode>(out_schema1, predicate,
                                                   table_info->oid_);
  }
  std::unique_ptr<AbstractPlanNode> insert_plan;
  {
    auto table_info =
        GetExecutorContext()->GetCatalog()->GetTable("empty_table2");
    insert_plan =
        std::make_unique<InsertPlanNode>(scan_plan1.get(), table_info->oid_);
  }
  GetExecutionEngine()->Execute(insert_plan.get(), nullptr, GetTxn(),
                                GetExecutorContext());
  ...
  const Schema *out_schema2;
  {
    ...
    scan_plan2 = std::make_unique<SeqScanPlanNode>(out_schema2, nullptr,
                                                   table_info->oid_);
  }
  std::vector<Tuple> result_set1;
  std::vector<Tuple> result_set2;
  GetExecutionEngine()->Execute(scan_plan1.get(), &result_set1, GetTxn(),
                                GetExecutorContext());
  GetExecutionEngine()->Execute(scan_plan2.get(), &result_set2, GetTxn(),
                                GetExecutorContext());

  ASSERT_EQ(result_set1.size(), result_set2.size());
  for (size_t i = 0; i < result_set1.size(); ++i) {
    ASSERT_EQ(result_set1[i]
                  .GetValue(out_schema1, out_schema1->GetColIdx("colA"))
                  .GetAs<int32_t>(),
              result_set2[i]
                  .GetValue(out_schema2, out_schema2->GetColIdx("colA"))
                  .GetAs<int32_t>());
    ASSERT_EQ(result_set1[i]
                  .GetValue(out_schema1, out_schema1->GetColIdx("colB"))
                  .GetAs<int32_t>(),
              result_set2[i]
                  .GetValue(out_schema2, out_schema2->GetColIdx("colB"))
                  .GetAs<int32_t>());
    std::cout << result_set1[i]
                     .GetValue(out_schema1, out_schema1->GetColIdx("colA"))
                     .GetAs<int32_t>()
              << ", "
              << result_set1[i]
...
TEST_F(ExecutorTest, DISABLED_SimpleRawInsertWithIndexTest) {
  ...
  auto index_info = GetExecutorContext()
                        ->GetCatalog()
                        ->CreateIndex<GenericKey<8>, RID, GenericComparator<8>>(
                            GetTxn(), "index1", "empty_table2",
                            table_info->schema_, *key_schema, {0}, 8);
  ...
  for (auto &table_tuple : result_set) {
    rids.clear();
    auto index_key = table_tuple.KeyFromTuple(
        schema, index_info->key_schema_, index_info->index_->GetKeyAttrs());
    index_info->index_->ScanKey(index_key, &rids, GetTxn());
    Tuple indexed_tuple;
    auto fetch_tuple =
        table_info->table_->GetTuple(rids[0], &indexed_tuple, GetTxn());

    ASSERT_TRUE(fetch_tuple);
    ASSERT_EQ(indexed_tuple.GetValue(out_schema, out_schema->GetColIdx("colA"))
                  .GetAs<int32_t>(),
              table_tuple.GetValue(out_schema, out_schema->GetColIdx("colA"))
                  .GetAs<int32_t>());
    ASSERT_EQ(indexed_tuple.GetValue(out_schema, out_schema->GetColIdx("colB"))
                  .GetAs<int32_t>(),
              table_tuple.GetValue(out_schema, out_schema->GetColIdx("colB"))
                  .GetAs<int32_t>());

    std::cout << indexed_tuple
                     .GetValue(out_schema, out_schema->GetColIdx("colA"))
                     .GetAs<int32_t>()
              << ", "
              << indexed_tuple
...
TEST_F(ExecutorTest, DISABLED_SimpleDeleteTest) {
  ...
  auto predicate =
      MakeComparisonExpression(colA, const50, ComparisonType::Equal);
  auto out_schema1 = MakeOutputSchema({{"colA", colA}});
  auto scan_plan1 = std::make_unique<SeqScanPlanNode>(out_schema1, predicate,
                                                      table_info->oid_);
  // index
  Schema *key_schema = ParseCreateStatement("a bigint");
  GenericComparator<8> comparator(key_schema);
  auto index_info =
      GetExecutorContext()
          ->GetCatalog()
          ->CreateIndex<GenericKey<8>, RID, GenericComparator<8>>(
              GetTxn(), "index1", "test_1",
              GetExecutorContext()->GetCatalog()->GetTable("test_1")->schema_,
              *key_schema, {0}, 8);

  // Execute
  std::vector<Tuple> result_set;
  GetExecutionEngine()->Execute(scan_plan1.get(), &result_set, GetTxn(),
                                GetExecutorContext());

  // Verify
  std::cout << "colA" << std::endl;
  for (const auto &tuple : result_set) {
    std::cout << tuple.GetValue(out_schema1, out_schema1->GetColIdx("colA"))
                     .GetAs<int32_t>()
              << std::endl;
    ASSERT_TRUE(tuple.GetValue(out_schema1, out_schema1->GetColIdx("colA"))
                    .GetAs<int32_t>() == 50);
  }
  ...
  {
    delete_plan =
        std::make_unique<DeletePlanNode>(scan_plan1.get(), table_info->oid_);
  }
  GetExecutionEngine()->Execute(delete_plan.get(), nullptr, GetTxn(),
                                GetExecutorContext());
...
TEST_F(ExecutorTest, DISABLED_SimpleNestedLoopJoinTest) {
  ...
  const Schema *out_schema1;
  {
    ...
    auto colB = MakeColumnValueExpression(schema, 0, "colB");
    out_schema1 = MakeOutputSchema({{"colA", colA}, {"colB", colB}});
    scan_plan1 = std::make_unique<SeqScanPlanNode>(out_schema1, nullptr,
                                                   table_info->oid_);
  }
  std::unique_ptr<AbstractPlanNode> scan_plan2;
  const Schema *out_schema2;
  ...
  {
    // colA and colB have a tuple index of 0 because they are the left side of
    // the join
    ...
    auto predicate =
        MakeComparisonExpression(colA, col1, ComparisonType::Equal);
    out_final = MakeOutputSchema(
        {{"colA", colA}, {"colB", colB}, {"col1", col1}, {"col3", col3}});
    join_plan = std::make_unique<NestedLoopJoinPlanNode>(
        out_final,
        std::vector<const AbstractPlanNode *>{scan_plan1.get(),
                                              scan_plan2.get()},
        predicate);
  }

  std::vector<Tuple> result_set;
  GetExecutionEngine()->Execute(join_plan.get(), &result_set, GetTxn(),
                                GetExecutorContext());
  ASSERT_EQ(result_set.size(), 100);
  std::cout << "ColA, ColB, Col1, Col3" << std::endl;
  for (const auto &tuple : result_set) {
    std::cout << tuple.GetValue(out_final, out_final->GetColIdx("colA"))
                     .GetAs<int32_t>()
              << ", "
              << tuple.GetValue(out_final, out_final->GetColIdx("colB"))
                     .GetAs<int32_t>()
              << ", "
              << tuple.GetValue(out_final, out_final->GetColIdx("col1"))
                     .GetAs<int16_t>()
              << ", "
              << tuple.GetValue(out_final, out_final->GetColIdx("col3"))
                     .GetAs<int32_t>()
              << ", " << std::endl;
  }
}
...
TEST_F(ExecutorTest, DISABLED_SimpleAggregationTest) {
  ...
  const Schema *scan_schema;
  {
    ...
    scan_plan = std::make_unique<SeqScanPlanNode>(scan_schema, nullptr,
                                                  table_info->oid_);
  }
  ...
  {
    const AbstractExpression *colA =
        MakeColumnValueExpression(*scan_schema, 0, "colA");
    ...
    agg_schema = MakeOutputSchema(
        {{"countA", countA}, {"sumA", sumA}, {"minA", minA}, {"maxA", maxA}});
    agg_plan = std::make_unique<AggregationPlanNode>(
        agg_schema, scan_plan.get(), nullptr,
  ...
  std::vector<Tuple> result_set;
  GetExecutionEngine()->Execute(agg_plan.get(), &result_set, GetTxn(),
                                GetExecutorContext());

  auto countA_val = result_set[0]
                        .GetValue(agg_schema, agg_schema->GetColIdx("countA"))
                        .GetAs<int32_t>();
  auto sumA_val = result_set[0]
                      .GetValue(agg_schema, agg_schema->GetColIdx("sumA"))
                      .GetAs<int32_t>();
  auto minA_val = result_set[0]
                      .GetValue(agg_schema, agg_schema->GetColIdx("minA"))
                      .GetAs<int32_t>();
  auto maxA_val = result_set[0]
                      .GetValue(agg_schema, agg_schema->GetColIdx("maxA"))
                      .GetAs<int32_t>();
...
TEST_F(ExecutorTest, DISABLED_SimpleGroupByAggregation) {
  // SELECT count(colA), colB, sum(colC) FROM test_1 Group By colB HAVING
  // count(colA) > 100
  std::unique_ptr<AbstractPlanNode> scan_plan;
  const Schema *scan_schema;
  {
    ...
    scan_schema =
        MakeOutputSchema({{"colA", colA}, {"colB", colB}, {"colC", colC}});
    scan_plan = std::make_unique<SeqScanPlanNode>(scan_schema, nullptr,
                                                  table_info->oid_);
  }
  ...
  const Schema *agg_schema;
  {
    const AbstractExpression *colA =
        MakeColumnValueExpression(*scan_schema, 0, "colA");
    const AbstractExpression *colB =
        MakeColumnValueExpression(*scan_schema, 0, "colB");
    const AbstractExpression *colC =
        MakeColumnValueExpression(*scan_schema, 0, "colC");
    // Make group bys
    ...
    const AbstractExpression *having = MakeComparisonExpression(
        countA, MakeConstantValueExpression(ValueFactory::GetIntegerValue(100)),
        ComparisonType::GreaterThan);

    // Create plan
    agg_schema = MakeOutputSchema(
        {{"countA", countA}, {"colB", groupbyB}, {"sumC", sumC}});
    agg_plan = std::make_unique<AggregationPlanNode>(
        agg_schema, scan_plan.get(), having, std::move(group_by_cols),
  ...
  for (const auto &tuple : result_set) {
    ...
    ASSERT_GT(tuple.GetValue(agg_schema, agg_schema->GetColIdx("countA"))
                  .GetAs<int32_t>(),
              100);
    // Should have unique colBs.
    auto colB = tuple.GetValue(agg_schema, agg_schema->GetColIdx("colB"))
                    .GetAs<int32_t>();
    ...
    std::cout << tuple.GetValue(agg_schema, agg_schema->GetColIdx("countA"))
                     .GetAs<int32_t>()
              << ", "
              << tuple.GetValue(agg_schema, agg_schema->GetColIdx("colB"))
                     .GetAs<int32_t>()
              << ", "
              << tuple.GetValue(agg_schema, agg_schema->GetColIdx("sumC"))
                     .GetAs<int32_t>()
              << std::endl;
  }
}
